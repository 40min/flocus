...
const mockedCreateDayTemplate =
  dayTemplateService.createDayTemplate as jest.Mock;
const mockedUpdateDayTemplate =
  dayTemplateService.updateDayTemplate as jest.Mock;
...
describe("EditTemplatePage", () => {
  ...
  describe("Create Mode", () => {
    beforeEach(() => {
      mockedUseTemplateById.mockReturnValue({
        data: undefined,
        isLoading: false,
        error: null,
      });
    ...
    it("renders create form", () => {
      ...
      expect(
        screen.getByText(
          "This template has no time windows yet. Add some below."
        )
      ).toBeInTheDocument();
    });
    ...
    it("allows adding and creating a new template with time windows", async () => {
      ...
      fireEvent.click(
        screen.getByRole("button", { name: "Add new time window" })
      );
      ...
      fireEvent.change(document.getElementById("twDescription")!, {
        target: { value: "New TW" },
      });
      ...
      await waitFor(() =>
        expect(
          screen.queryByRole("dialog", { name: /add new time window/i })
      ...
      fireEvent.change(screen.getByLabelText("Template Name"), {
        target: { value: "New Template With TW" },
      });
  ...
  describe("Edit Mode", () => {
    ...
    it("renders edit form with pre-filled data and time windows", () => {
      ...
      expect(
        screen.getByRole("heading", { name: /Edit Template/i, level: 1 })
      ).toBeInTheDocument();
      expect(screen.getByLabelText("Template Name")).toHaveValue(
        mockTemplate.name
      );
      expect(
        screen.getByText(mockTemplate.time_windows[0].category.name)
      ).toBeInTheDocument();
    ...
    it("allows updating template details", async () => {
      ...
      await waitFor(() => {
        ...
        ).toBeInTheDocument(); // Button should show 'Saved'
      });
    });
    ...
    it("allows adding a new time window to an existing template", async () => {
      ...
      expect(screen.getAllByText("Evening Read").length).toBeGreaterThan(0);
      expect(screen.getAllByText("Personal").length).toBeGreaterThan(0);
      expect(screen.getAllByText("(20:00 - 21:00)").length).toBeGreaterThan(0);

      fireEvent.click(screen.getByRole("button", { name: "Save Changes" }));

      await waitFor(() => {
        expect(mockedUpdateDayTemplate).toHaveBeenCalled();
        // Verify that the update was called with the template ID
        expect(mockedUpdateDayTemplate).toHaveBeenCalledWith(
          "template1",
    ...
    it("allows editing an existing time window", async () => {
      mockedUpdateDayTemplate.mockResolvedValue({
        ...
        time_windows: [
          {
            id: "tw1",
            description: "Updated Morning work",
            start_time: 540,
            end_time: 600,
            category: mockCategories[0],
            day_template_id: "template1",
      ...
      const modal = await screen.findByRole("dialog", {
        name: /edit time window/i,
      });

      expect(document.getElementById("twDescription")).toHaveValue(
        "Morning work"
      );
      ...
      fireEvent.change(document.getElementById("twDescription")!, {
        target: { value: "Updated Morning work" },
      });
      fireEvent.change(screen.getByLabelText("End Time"), {
        target: { value: "10:00" },
      }); // Change end time
      fireEvent.click(
        screen.getByRole("button", { name: "Update Time Window" })
      );

      await waitFor(() =>
        expect(
          screen.queryByRole("dialog", { name: /edit time window/i })
        ).not.toBeInTheDocument()
      );
    ...
    it("handles template not found error", () => {
      mockedUseTemplateById.mockReturnValue({
        data: undefined,
        isLoading: false,
        error: new NotFoundError("Template not found"),
      });
