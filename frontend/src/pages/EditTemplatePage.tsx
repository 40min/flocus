...
const EditTemplatePage: React.FC = () => {
  ...
  const {
    ...
    error: templateError,
  } = useTemplateById(routeTemplateId);
  ...
  const hasTimeWindowChanges = useCallback(() => {
    ...
    const sortedCurrent = [...templateTimeWindows].sort((a, b) =>
      a.id.localeCompare(b.id)
    );
    const sortedInitial = [...initialTimeWindows].sort((a, b) =>
      a.id.localeCompare(b.id)
    );
  ...
  const updateTemplateMutation = useMutation({
    mutationFn: ({
      ...
      id: string;
      data: DayTemplateUpdateRequest;
    }) => updateDayTemplate(id, data),
    onSuccess: (savedTemplate: DayTemplateResponse) => {
      queryClient.invalidateQueries({ queryKey: ["templates"] });
      queryClient.invalidateQueries({
        queryKey: ["template", savedTemplate.id],
      });
    ...
    <div className="p-8 @container mx-auto max-w-4xl">
        ...
        >
          <ol className="list-none p-0 inline-flex">
            <li className="flex items-center">
              <a href="/templates" className="hover:text-gray-700">
                Templates
              </a>
            </li>
            <li className="flex items-center">
      ...
      <header className="mb-8">
        ...
        <p className="text-gray-600 text-sm mt-1">
          {isCreatingNew
            ? "Define a new template for your days."
            : "Modify the template details and manage time windows."}
        </p>
      </header>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Manage time windows for this template. Add or remove as needed.
          </p>
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {templateTimeWindows.length > 0 ? (
              templateTimeWindows
                .slice()
                    ...
                    <div className="flex-grow min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-400 text-xs">
                          ({formatMinutesToHHMM(tw.start_time)} -{" "}
                          {formatMinutesToHHMM(tw.end_time)})
                        </span>
                      </div>
                      {tw.description && (
        ...
        <div className="flex justify-end gap-3 mt-8">
          ...
          <Button
            type="submit"
            variant={
              hasUnsavedChanges || isCreatingNew ? "primary" : "secondary"
            }
            className={
              hasUnsavedChanges
                ? "bg-green-500 hover:bg-green-600 save-button-unsaved"
